...
### PERINGATAN !!
Baileys ini telah sedikit dimodifikasi oleh zarv7z. Jangan gunakan untuk kegiatan ilegal atau untuk merugikan orang lain.

### CATATAN PENTING

Perpustakaan ini awalnya merupakan proyek dari MzArth Staff Davelopement dan tidak ada hubungan dengan atau disetujui oleh WhatsApp. Gunakan dengan kebijaksanaan Anda sendiri. Jangan spam orang lain dengan ini. Kami tidak mendorong penggunaan stalking, pesan massal atau otomatis.

### PEMBERITAHUAN KEWAJIBAN DAN LISENSI
BaileysPro tidak memerlukan Selenium atau browser lain untuk berinteraksi dengan WhatsApp Web, melainkan langsung menggunakan WebSocket. Tidak menjalankan Selenium atau Chromimum dapat menghemat setengah gigabyte RAM :/ BaileysPro mendukung interaksi dengan multi-device dan versi web WhatsApp.
##

### TOLONG BACA
Repositori asli harus dihapus oleh penulis asli kami sekarang melanjutkan pengembangan repositori ini di sini. Ini adalah salah satu repositori asli milik MzArth Staff Davelopment. di modifikasi dan dipelihara oleh zarv7z & Lin'h

![My Card ](https://cardivo.vercel.app/api?name=LyrraEvanthMeta%20&description=”aku%20hanya%20bisa%20memprogram%20nya,%20jika%20kodenya%20sudah%20benar,%20program%20itu%20akan%20berjalan%20sesuai,%20keinginan%20pemilik%20aslinya”%20(%20tapi%20ini%20bukan%20soal%20programnya%20)&image=https://b.top4top.io/p_2090f6xvx0.jpg&backgroundColor=%23ecf0f1&instagram=@Lyrra-Evanth&github=Lyrra&pattern=leaf&colorPattern=%23eaeaea)
...
```
const baileys = require("@Lyrra-Evanth/Meta-Libray");
...
## send all buttons interactive:
 ```
const { generateWAMessageFromContent, proto } = require("@LyrraEvanth/MetaLibray")
...
```
yarn add @LyrraEvanth/MetaLibray
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:Lyrra/BaileysV2
```

Then import your code using:
``` ts 
import makeWASocket from '@LyrraEvanth/MetaLibray'
```
...
``` ts
import makeWASocket, { DisconnectReason } from '@LyrraEvanth/MetaLibray'
...
``` ts
import makeWASocket, { BufferJSON, useMultiFileAuthState } from '@LyrraEvanth/MetaLibray'
...
``` ts
import makeWASocket, { makeInMemoryStore } from '@LyrraEvanth/MetaLibray'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from '@LyrraEvanth/MetaLibray'
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from '@LyrraEvanth/MetaLibray'
                                */
...
import { downloadMediaMessage } from '@LyrraEvanth/MetaLibray'
